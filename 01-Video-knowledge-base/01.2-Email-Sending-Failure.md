...
# 🎥 Email Sending Failure — Video Script

**Video Knowledge Base — Script 01.2**
...
flowchart TD
    subgraph R1[" "]
        direction LR
        A["🔎 Identify"]:::s1 --> B["🧪 Diagnose"]:::s2 --> C["🛠 Fix"]:::s3
    end
    subgraph R2[" "]
        direction LR
        D["✅ Verify"]:::s4 --> E["👤 Close"]:::s5
    end
    R1 --> R2
...
```
<p align="center"><em>Identify → Diagnose → Fix, then Verify → Close.</em></p>
...
flowchart TD
    subgraph R1[" "]
        direction LR
        A["💻 Client"]:::a --> B["🔌 Port/TLS"]:::b --> C["🔑 Auth"]:::c
    end
    subgraph R2[" "]
        direction LR
        D["🖥 SMTP server"]:::d --> E["📬 Recipient"]:::e
    end
    R1 --> R2
...
```mermaid
flowchart TD
    A["🚨 Test email fails"]:::alert --> B["🔎 Read new error"]:::work
    B --> C["🔌 Test port"]:::work
    C -->|Blocked| D["🌐 ISP / firewall"]:::work
    C -->|Open| E["🖥 Provider / account"]:::work
    D --> F["📨 Escalate"]:::done
    E --> F
...
```mermaid
flowchart TD
    Start(["🚨 Email Not Sending"]):::start --> A{"Receiving OK?"}:::step
    A -->|No| B(["📨 Escalate"]):::esc
    A -->|Yes| C["Check error + port"]:::step
    C --> D["Fix port"]:::fix
    D --> E["Fix auth"]:::fix
    E --> G["Send test"]:::step
