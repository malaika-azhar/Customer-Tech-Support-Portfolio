...
# 🎥 VPN Authentication Failure — Video Script

**Video Knowledge Base — Script 01.3**
...
flowchart TD
    subgraph R1[" "]
        direction LR
        A["💻 VPN client"]:::a --> B["🚪 VPN gateway"]:::b
    end
    subgraph R2[" "]
        direction LR
        C["🔑 Account directory"]:::c --> D["🏢 Internal resources"]:::d
    end
    R1 --> R2
...
```mermaid
flowchart TD
    A["🚨 Account locked"]:::alert --> B["🔎 Check lockout source"]:::work
    B -->|Own device| C["🔧 Update password"]:::done
    B -->|Unknown source| D["🛑 Reset + escalate"]:::alert
    B -->|Many accounts| E["🛑 Possible spraying"]:::alert
...
```mermaid
flowchart TD
    Start(["🚨 VPN Auth Failed"]):::start --> V["Verify identity"]:::fix
    V --> B{"Account locked?"}:::step
    B -->|No| D(["📨 Escalate"]):::esc
    B -->|Yes| S["Find source of attempts"]:::step
    S -->|Own device| C["Unlock + fix old password"]:::fix
    S -->|Unknown| X(["🛑 Reset + escalate"]):::esc
    C --> End1(["Resolved ✅"]):::ok
