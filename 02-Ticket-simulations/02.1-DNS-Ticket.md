...
%%{init: { 'theme': 'base', 'themeVariables': {
  'activeTaskBkgColor':'#2E86C1', 'activeTaskBorderColor':'#1B4F72',
  'doneTaskBkgColor':'#17A589', 'doneTaskBorderColor':'#0E6655',
  'critBkgColor':'#E67E22', 'critBorderColor':'#A04000',
  'sectionBkgColor':'#EBF5FB', 'altSectionBkgColor':'#FEF5E7',
  'taskTextColor':'#FFFFFF', 'taskTextOutsideColor':'#1B2631',
  'taskTextLightColor':'#FFFFFF',
  'titleColor':'#1B2A4A', 'fontSize':'16px'
}}}%%
gantt
    title Project Flow - DNS Resolution Ticket
    dateFormat X
    axisFormat %s
    section TKT-2026-001 DNS
    Triage and Scope          :active, 0, 1
    Diagnose on Server        :crit, 1, 2
    Restart DNS Service       :done, 2, 3
    Verify with Real Setup    :done, 3, 4
```
<p align="center"><em>Blue = triage, orange = diagnosis (proves the cause), teal = fix and verification.</em></p>
...
```mermaid
flowchart LR
    A["🛠 Server fixed"]:::work --> B{"Client DNS on 8.8.8.8?"}:::ask
    B -->|Yes| C["🛑 Proves nothing"]:::bad
    B -->|No| D["✅ Real proof"]:::good

    classDef work fill:#2C3E70,stroke:#131B3A,stroke-width:2px,color:#FFFFFF,font-weight:bold
    classDef ask fill:#B9770E,stroke:#6E4409,stroke-width:2px,color:#FFFFFF,font-weight:bold
    classDef bad fill:#943126,stroke:#571C16,stroke-width:2px,color:#FFFFFF,font-weight:bold
    classDef good fill:#1E8449,stroke:#0E4A28,stroke-width:2px,color:#FFFFFF,font-weight:bold
```
